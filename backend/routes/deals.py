...
def complete_deal(data: DealComplete):
    ...
    try:
        ...
        # Remove the product listing from the active marketplace
        # by marking it as sold. The product row is preserved
        # so completed transactions remain available for history.
        connection.execute(
            """
            UPDATE products
            SET status = 'sold'
        ...
        return {
            "success": True,
            "message": "Deal completed and product removed from marketplace"
        }
